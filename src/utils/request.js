...
service.interceptors.response.use(
  response => {
    ...
    if (res.code !== 20000) { // 后端自定义异常(比如上传密钥错误等)
      Message({
        message: res.message,
        type: 'error',
        duration: 5 * 1000
      ...
      return Promise.reject('error') // 返回error错误
    } else {
      return response.data // 成功的返回结果
    }
  },
  error => { // Http错误(比如请求地址错误或服务器没启动等)
    console.log('err:' + error) // 打印结果err:error
    Message({
